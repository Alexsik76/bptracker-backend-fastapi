...
| Метод | Шлях | Тіло запиту | Відповідь | Помилки |
|---|---|---|---|---|
| GET | `/users/me` | — | 200, `{id, email, created_at}` | 404 |
...
| Метод | Шлях | Тіло запиту | Відповідь | Помилки |
|---|---|---|---|---|
| POST | `/export/csv` | `{tz:str}` | 202, `{message, email}` | 404 — user not found; 422 — невалідний timezone; 429 — cooldown active |

`tz` — обов'язковий IANA-ідентифікатор часового поясу (напр. `"Europe/Kyiv"`).
Бек форматує дати в CSV відповідно до цього поясу. Файл надсилається на email
користувача через email outbox (асинхронно, з ретраями). Cooldown між
експортами — `export_cooldown_minutes` (за замовчуванням 10 хв).
...
| users | ✓ профіль + видалення акаунта, покрито тестами |
...
| export | ✓ CSV-експорт на email, покрито тестами |
...
`scripts/import_from_legacy.py` — одноразовий інструмент для перенесення даних
з legacy C#-бекенду (Postgres) в нову FastAPI-базу. Переносить лише дві таблиці:
...
Решта таблиць (`TreatmentSchemas`, `IntakeReports`, `ReminderTemplates` тощо)
**не** мігрується — причини описані в коментарях до скрипта.

### Запуск

Скрипт повністю автономний — не залежить від `.env` чи `config.py`. Обидва
URL передаються через CLI, тому запускати можна з будь-якої машини, яка має
мережевий доступ до обох баз:
...
- `--legacy-url` — рядок підключення до legacy (джерело).
- `--target-url` — рядок підключення до нової бази (ціль).
- `--user-id` — UUID користувача в **новій** базі. Скрипт не створює
  користувача; якщо його не існує — abort.
- `--dry-run` — прочитати джерело, перевірити цільового користувача, показати
  кількості рядків, **нічого не записувати**.

### Ідемпотентність

Скрипт перевіряє, чи є вже рядки в `measurements` або `webauthn_credentials`
для переданого `user_id`. Якщо є — abort із `DELETE`-командами, які потрібно
виконати вручну. Скрипт ніколи не видаляє дані сам і не створює дублікати.
